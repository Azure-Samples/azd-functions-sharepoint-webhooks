...
+ Be `Owner` of the subscription (or have the role [`Role Based Access Control Administrator`](https://learn.microsoft.com/en-us/azure/role-based-access-control/built-in-roles/privileged#role-based-access-control-administrator)), to assign Azure RBAC roles to the managed identity (action `Microsoft.Authorization/roleAssignments/write`) when provisioning the resources in Azure
